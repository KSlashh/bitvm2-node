...
use goat::{
    constants::{OPERATOR_ACK_TIMELOCK, OPERATOR_COMMIT_TIMELOCK, WATCHTOWER_CHALLENGE_TIMELOCK},
    transactions::base::output_topology,
...
pub async fn process_graph_challenge(
    ...
    for graph in graphs {
        ...
        // Todo: vanhger
        if let Some((actor, message_content)) =
            detect_operator_commit_pubin(btc_client, local_db, &graph).await?
        {
            info!("process_graph_challenge detect operator commit pubin confirmed on-chain");
            let mut storage_processor = local_db.acquire().await?;
            upsert_message(
                &mut storage_processor,
                false,
...
async fn detect_watchtower_flow(
        ...
        match watchtower_spent_txid {
            Some(_) if !watchtower_timeout_spent => {
                ...
                if ack_spent_txid.is_none()
                    ...
                    && is_timelock_ready(
                        monitor.height,
                        num_blocks_per_network(get_network(), OPERATOR_ACK_TIMELOCK) as i64,
                        current_height,
                    )
                {
                    any_nack_ready = true;
            ...
            None => {
                ...
                if ack_spent_txid.is_none()
                    && is_timelock_ready(
                        monitor.height,
                        num_blocks_per_network(get_network(), WATCHTOWER_CHALLENGE_TIMELOCK) as i64,
                        current_height,
                    )
                {
        ...
        && connector_f_spent_txid.is_none()
    {
        ...
        if is_timelock_ready(
            monitor.height,
            num_blocks_per_network(get_network(), OPERATOR_COMMIT_TIMELOCK) as i64,
            current_height,
        ) {
            messages.push(DetectedGraphMessage::new(
                Actor::Verifier,
...
async fn detect_operator_commit_pubin(
    btc_client: &BTCClient,
    local_db: &LocalDB,
    graph: &Graph,
    ...
    let watchtower_challenge_init_txid: Txid = match graph.watchtower_challenge_init_txid.clone() {
        ...
        None => {
            warn!(
                "detect_operator_commit_pubin graph_id:{} watchtower_challenge_init_txid is none",
                graph.graph_id
            );
            return Ok(None);
        }
    ...
    // vout_len is stored in graph_btc_tx_vout_monitor by detect_watchtower_challenge
    let vout_len = {
        let mut storage_processor = local_db.acquire().await?;
        storage_processor
    ...
    // ConnectorE is the 3rd-to-last output
    // (layout: ..., connector_e, connector_f, anchor)
    let connector_e_vout = (vout_len - 3) as u64;
    let Some(spend_txid) =
        outpoint_spent_txid(btc_client, &watchtower_challenge_init_txid, connector_e_vout).await?
    else {
        return Ok(None);
    ...
    // if ConnectorE is spent by operator_commit_timeout — skip
    let commit_timeout_txid: Option<Txid> =
        graph.operator_commit_timeout_txid.clone().map(Txid::from);
    if commit_timeout_txid == Some(spend_txid) {
        return Ok(None);
    }
...
mod detect_commit_pubin_tests {
    ...
    fn make_dummy_graph(
        graph_id: Uuid,
        instance_id: Uuid,
        watchtower_challenge_init_txid: Option<SerializableTxid>,
    ...
    async fn insert_vout_for_watchtower(db: &LocalDB, graph_id: &Uuid, txid: Txid, vout_len: i64) {
        let mut s = db.acquire().await.unwrap();
        s.upsert_graph_btc_tx_vout_monitor(&GraphBtcTxVoutMonitor {
            graph_id: *graph_id,
            tx_name: "watchtower_init".into(),
            txid: SerializableTxid::from(txid),
            height: 100,
            vout_len,
            monitor_data: "".into(),
            created_at: 0,
    ...
    #[tokio::test]
    async fn returns_none_when_init_txid_missing() {
        let db = create_local_db("sqlite::memory:").await;
        let (btc_client, _) = BTCClient::new_mock_client();

        let graph = make_dummy_graph(Uuid::new_v4(), Uuid::new_v4(), None, None);
        let result = detect_operator_commit_pubin(&btc_client, &db, &graph).await.unwrap();
        assert!(result.is_none());
    }

    #[tokio::test]
    async fn returns_none_when_vout_monitor_missing_from_db() {
        let db = create_local_db("sqlite::memory:").await;
        let (btc_client, _) = BTCClient::new_mock_client();

        let init_txid = make_txid(0x01);
        let graph = make_dummy_graph(
            Uuid::new_v4(),
            Uuid::new_v4(),
            Some(SerializableTxid::from(init_txid)),
            None,
        );
        // vout_monitor not inserted → vout_len defaults to 0 → returns None
        let result = detect_operator_commit_pubin(&btc_client, &db, &graph).await.unwrap();
        assert!(result.is_none());
    }

    #[tokio::test]
    async fn returns_none_when_connector_e_not_spent() {
        let db = create_local_db("sqlite::memory:").await;
        let (btc_client, _) = BTCClient::new_mock_client();

        let graph_id = Uuid::new_v4();
        let init_txid = make_txid(0x01);
        insert_vout_for_watchtower(&db, &graph_id, init_txid, 5).await;
    ...
    #[tokio::test]
    async fn returns_none_when_connector_e_spent_by_commit_timeout() {
        let db = create_local_db("sqlite::memory:").await;
        let (btc_client, mock_adaptor) = BTCClient::new_mock_client();

        let graph_id = Uuid::new_v4();
        let init_txid = make_txid(0x01);
        let timeout_txid = make_txid(0x02);
        let connector_e_vout = 2u64;
        insert_vout_for_watchtower(&db, &graph_id, init_txid, 5).await;
        mock_adaptor.set_output_status(
            init_txid,
            connector_e_vout,
            OutputStatus { spent: true, txid: Some(timeout_txid), vin: Some(0), status: None },
        );
        ...
        let graph = make_dummy_graph(
            ...
            Uuid::new_v4(),
            Some(SerializableTxid::from(init_txid)),
            Some(SerializableTxid::from(timeout_txid)),
        );
        let result = detect_operator_commit_pubin(&btc_client, &db, &graph).await.unwrap();
        assert!(result.is_none());
    }
    ...
    #[tokio::test]
    async fn returns_assert_ready_when_connector_e_spent_by_commit_pubin() {
        let db = create_local_db("sqlite::memory:").await;
        let (btc_client, mock_adaptor) = BTCClient::new_mock_client();
        ...
        let init_txid = make_txid(0x01);
        let commit_pubin_txid = make_txid(0x03);
        let timeout_txid = make_txid(0x02);
        let connector_e_vout = 2u64;
        insert_vout_for_watchtower(&db, &graph_id, init_txid, 5).await;
        mock_adaptor.set_output_status(
            ...
            OutputStatus {
                spent: true,
                txid: Some(commit_pubin_txid), // spend by commit txn
                vin: Some(0),
                status: None,
            },
        );
        ...
        let (actor, content) = result.expect("should detect commit-pubin");
        assert_eq!(actor, Actor::Operator);
        match content {
            GOATMessageContent::AssertReady(msg) => {
                assert_eq!(msg.instance_id, instance_id);
                assert_eq!(msg.graph_id, graph_id);
            }
            _ => panic!("expected AssertReady"),
        }
    }
}
